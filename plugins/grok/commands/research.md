...
argument-hint: '[--model <model>] [--effort <low|medium|high|xhigh|max>] [--no-web-search] [--timeout <duration>] <question>'
...
- **`--check`** is on by default — Grok appends a self-verification loop to its own answer.
